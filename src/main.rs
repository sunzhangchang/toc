...
struct Toc {
    ...
    files: Vec<String>, // all V files that need to be parsed and compiled
...
impl Toc {
    fn new(dir: &str, target_os_: &str, out_name: &str) -> Toc {
        let mut dir = Path::new(&dir);
        // println('new compiler "$dir"')
        let mut out_name = out_name.to_string();
        // build mode
        ...
        */
        ...
        // }
        //
        let is_test = dir.file_name().unwrap().to_str().unwrap() == "_test.toc";
        let is_script = dir.ends_with(".toc");

        if is_script && !dir.exists() {
            println!("'{}' does not exist!", dir.to_str().unwrap());
            exit(1)
        }
        // No -o provided? foo.v => foo
        if out_name == "a.out" && dir.ends_with(".toc") {
            out_name = dir.file_stem().unwrap().to_str().unwrap().to_string();
        }
        // if we are in `/foo` and run `v .`, the executable should be `foo`
        if dir.file_name().unwrap().to_str().unwrap() == "." && out_name == "a.out" {
            let tmp = env::current_dir().unwrap();
            let base = tmp.to_str().unwrap();
            out_name = base.trim().to_string();
        }
        let mut _os = OS::Mac;
        ...
        let mut lang_dir = "".to_string();
        // First try fetching it from VROOT if it's defined
        (|| {
            let toc_root_path = Path::new(TMP_PATH).join("/TOC_ROOT");
            if toc_root_path.exists() {
                let toc_root = fs::read(toc_root_path);
                if let Ok(data) = toc_root {
                    let tmp = String::from_utf8(data).unwrap();
                    let tr = Path::new(tmp.trim());
                    if tr.exists() && tr.join("/vlib/builtin").exists() {
                        lang_dir = tr.to_str().unwrap().to_string();
                    }
                } else {
                    return;
                }
        ...
        //     }
        // }
        let out_name_c = out_name.to_string() + "/" + ".cxx";
        // mut files := []string
        // // Add builtin files
...
fn main() {
    let app = Command::new("toc")
        .version(VERSION)
        .arg(Arg::new("file_name").takes_value(true));
    let matches = app.get_matches();

    let tm = matches.subcommand();
    if let Some((t, ap)) = tm {
        println!("{} {:?}", t, ap);
    } else {
        if let Some(file_name) = matches.value_of("file_name") {
            let toc = Toc::new(file_name, "", "");
            println!("{}", toc.dir);
        } else {
            // run_repl();
            println!("!@#");
